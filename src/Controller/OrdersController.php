...
class OrdersController extends AbstractController
{
    ...
    function addOrder(SessionInterface $session, ProductsRepository $repo, OrdersRepository $ordersRepository, UserRepository $userRepo)
    {
    ...
    function confirmOrder(SessionInterface $session, ProductsRepository $repo, OrdersRepository $ordersRepository, UserRepository $userRepo)
    {
        $cart = $session->get("cart", []);
        $dataCart = [];
        $total = 0;
        ...
        foreach ($cart as $id => $qty) {
            ...
            if ($qty > 0) {
                ...
                $user = $userRepo->find($user1->getId());
                $order = new Orders();
                // $order->setTotal($product->getPrice() * $qty);
